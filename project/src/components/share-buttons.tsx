...
export function ShareButtons({ copy, language }: { copy: ShareButtonsCopy; language: AppLanguage }) {
  ...
  async function handlePosterShare() {
    trackEvent("share_link_clicked", { platform: "poster" });
    const loadingId = toast.loading(copy.posterPreparingToast);
    ...
    try {
      ...
      toast.error(copy.posterShareFailed, { id: loadingId });
      return;
    }
    ...
    if (canShareFiles && typeof navigator.share === "function") {
      try {
        await navigator.share({ files: [file], text: copy.posterCaptionText });
      } catch (error) {
        if ((error as { name?: string }).name !== "AbortError") {
          console.error("navigator.share failed (poster)", error);
          toast.error(copy.posterShareFailed);
        }
      }
      return;
    }

    // Desktop / unsupported fallback: no OS share sheet exists here, so hand
    // the user both halves manually — the image as a download, the caption
    // on their clipboard — matching what a native share sheet would have done.
    ...
    try {
      await navigator.clipboard.writeText(copy.posterCaptionText);
      toast.success(copy.posterFallbackToast);
    } catch (error) {
      console.error("navigator.clipboard.writeText failed (poster caption)", error);
      toast.error(copy.linkCopyFailed);
    }
  }
  ...
  return (
    <div className="grid gap-2">
      ...
      <button
        ...
        className="flex h-11 items-center gap-2.5 rounded-sm border-2 border-border bg-card px-4 text-sm font-bold text-foreground transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
        <Images className="size-4" strokeWidth={1.5} aria-hidden="true" />
        {copy.sharePoster}
      </button>
      <button
        type="button"
