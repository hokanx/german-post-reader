...
- Google Gemini (gemini-3.6-flash, paid tier — pinned to an explicit stable version, never the `-latest` alias): Vision + document model: reads letter images AND PDFs directly (native multimodal document understanding — no separate OCR step needed for either), returns structured JSON with summary, deadlines, and reply draft in the user's chosen language. `-latest` was tried first for its free tier, but it silently repointed at a newer model with a 20-request/day free-tier cap and broke production with no code change — paid tier is required both to remove that cap and because Google's free tier allows human review of inputs/outputs for training, which isn't acceptable for the official government/bank/insurance letters this app handles. Tesseract.js was dropped from the stack: it was originally scoped as the PDF-text-extraction step for the OpenAI pipeline this was swapped in from, but Tesseract/Leptonica cannot decode PDF containers at all (image formats only), and Gemini's native PDF support makes that step unnecessary regardless.
...
### AI pipeline rules
- The analysis server action MUST return structured JSON with these exact keys: `summary` (string — must NOT name the sender; that's `sender_name`'s job and it's shown in its own dedicated section, so repeating it in `summary` is a duplicate), `sender_name` (string — the sender exactly as printed, never translated), `sender_category` (one of `authority | insurer | bank | landlord | utility | school | delivery | other` — drives the sender icon on the dashboard card and the letter-detail page), `letter_date` (string, ISO 8601 — the date the letter itself is dated/issued by the sender, e.g. "München, den 15.03.2026"; distinct from `letters.created_at`, which is upload time, not send time; empty string if no such date is printed or determinable), `deadlines` (array of {date, description}), `payments` (array of {description, amount, source_quote} — EVERY payment amount or payment change the letter states; must never be missed, shown in its own section on the letter-detail page), `appointments` (array of {description, date, source_quote} — every fixed date/time the recipient must physically attend, distinct from `deadlines`, shown in the same section), `key_facts` (array of {label, value, source_quote} — source_quote always in the original German), `action_required` (boolean), `reply_draft` (string), `reply_draft_translation` (string), `detected_language_confirmed` (boolean), `risk_flags` (array of strings for ambiguous amounts or dates).
